...
def infer_on_image(face_detection_cnn,input_image):
    """
    Run inference on image
    :params face_detection_cnn Boolean If true it will run face detection using CNN if False will use HOG Detector
    :params input_image Input image to run inference on
    ...
    image_frame = cv2.imread(input_image)
    ...
    model = models.load_model("D:\\Projects\\FaceMaskDetection\\h5_weigth\\mnm_model.h5")
    ...
    for single_box in boxes:
        ...
        try:
            face_resized_img = image_frame[y:y1,x:x1]
            ...
            cv2.rectangle(image_frame,(x,y-20),(x1,y),(255,255,225),-1)
            cv2.putText(image_frame,class_name, (x , y-10),cv2.FONT_HERSHEY_SIMPLEX,
                    0.5,(0,0,0),1,cv2.LINE_AA)
            cv2.rectangle(image_frame,(x,y),(x1,y1),(0,255,0),2)
        except:
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument('--inp_img','-img',help='Pass image as an input. ',required=True)

    args = parser.parse_args()
    infer_on_image(args.face_cnn,args.inp_img)